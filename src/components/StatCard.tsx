export default function StatCard({
  ...
  return (
    <div className="group rounded-2xl border border-slate-200/60 bg-white p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md hover:border-slate-300/80">
      ...
      {loading ? (
        <>
          <div className="mt-3 h-9 w-16 animate-pulse rounded-lg bg-slate-200/80" />
          <div className="mt-4 h-6 w-36 animate-pulse rounded-md bg-slate-100" />
        </>
      ) : (
        <>
          ...
          <p className="mt-2.5 text-xs font-medium text-slate-500 bg-slate-50 inline-block px-2 py-1 rounded-md">
            {detail}
          </p>
        </>
      )}
